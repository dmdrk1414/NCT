...
@Data
@Service
public class GoogleOAuthLoginApiClientService implements
    OAuthLoginApiClientService<GoogleOAuthProfile> {
  ...
  private void init() {
    restTemplate = new RestTemplateBuilder().errorHandler(new RestTemplateResponseErrorHandler())
        .build();
  ...
  public String getGoogleTokenInfoURI(String accessToken) {
    return UriComponentsBuilder.fromHttpUrl(getGoogleAuthUrl() + "/tokeninfo")
  ...
  private String requestOAuthClientAccessToken(String authCode) throws JsonProcessingException {
    ...
    ResponseEntity<String> apiResponseString = restTemplate.postForEntity(getTokenURI(),
        httpRequestEntity, String.class);
    GoogleOAuthLoginResDto googleOAuthLoginResDto = objectMapper.readValue(
        apiResponseString.getBody(), new TypeReference<GoogleOAuthLoginResDto>() {
        });
  ...
  private GoogleOAuthProfile requestOauthInfo(String accessToken) throws JsonProcessingException {
    ...
    GoogleOAuthTokenInfoResDto response = objectMapper.readValue(resultTokenInfo,
        new TypeReference<GoogleOAuthTokenInfoResDto>() {
        });
